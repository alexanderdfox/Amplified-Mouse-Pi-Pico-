...
# UART config packet: 0x55 0xCF 0x01 N L I A Q_lo Q_hi [save]
...
def build_packet(num_mice: int, logic_mode: int, input_mode: int,
    ...
    return bytes([
        ...
        num_mice & 0xFF, logic_mode & 0xFF, input_mode & 0xFF,
...
def main() -> None:
    ...
    packet = build_packet(num_mice, logic_mode, input_mode, amplify, quad_scale, save=not args.no_save)
    with serial.Serial(args.port, args.baud, timeout=1) as ser:
        ser.write(packet)
    print(f"Sent settings to {args.port}: num_mice={num_mice} logic={logic_mode} input={input_mode} amplify={amplify} quad_scale={quad_scale} save={not args.no_save}")
